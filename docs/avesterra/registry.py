...
def create_registry(name, autosave, auth, *, outlet=None, server=None):
    """
    create_registry(name, autosave, auth, *, outlet=None, server=None)
    ...
    outlet : Entity, optional
        the outlet that is responsible for maintaining this registry.  If not provided, the registry outlet is connected to
        the entity. 

    server : Entity, optional
        the server on which the new registry is to be created. If not provided,
    ...
    return av.create_entity(
        ...
        outlet=outlet if outlet else av.registry_outlet(),
        server=server, autosave=autosave, handle_exceptions=False)

@av.manage_exceptions('while deleting registry {0}')
def delete_registry(registry, auth):
    """
    delete_registry(registry, auth)
    ...
    for entity in registry_entities(registry, auth):
...
def registry_entities(registry, auth=None, handle_exceptions=False, 
                      full_details=False):
    """Create a generator of entities in a registry."""
    registry_details = av.retrieve_entity(
        registry, auth, handle_exceptions=handle_exceptions)
...
def register_entity(registry, name, key, entity, auth):
    """
    register_entity(registry, name, key, entity, auth)
    ...
    replaced_entity = av.invoke(
        registry, av.Method.UPDATE, auth, name=name, key=key, value=entity, 
        return_results=True)
...
def deregister_entity(registry, auth, *, index=None, key=None):
    """
    deregister_entity(registry, auth, *, index=None, key=None)
    ...
    av.invoke(
        registry, av.Method.REMOVE, auth, index=index, key=key)
    if entity != atapi.NULL_ENTITY:
        av.dereference_entity(entity, auth, handle_exceptions=False)
        _release_entity(entity, auth)

def erase_registry(*args, **kwargs):
...
def lookup_registry(registry, key, auth=None):
    """
    lookup_registry(registry, key, auth=None)
    ...
    auth : Authorization
        an object that authorizes looking up a key in a registry
    ...
    with av.suppress_console_error_logging(
        ...
        try:
            ent = av.invoke(
                registry, av.Method.LOOKUP, auth, key=key, return_results=True)
            return True, ent
...
def find_registry(registry, name, auth=None, *, index=None):
    """
    find_registry(registry, name, auth=None, *, index=None)
    Return the entity with a particular name.""" 
    Verify.registry(registry)
    return av.find_property(
        registry, name, auth, index=index, handle_exceptions=False)

@av.manage_exceptions('while finding item in registry {0}')
def registry_item(registry, auth=None, *, index=None):
    """
    registry_item(registry, auth=None, *, index=None)
    Return the entity at a particular index position in the registry.""" 
    Verify.registry(registry)
    item = av.property_value(registry, auth, index=index)
    Verify.entity(item)
    return item

@av.manage_exceptions('while counting entities in registry {0}')
def item_count(registry, auth=None):
    """
    item_count(registry, auth=None)
    Return count of entities in the registry.""" 
    Verify.registry(registry)
    return av.property_count(registry, auth, handle_exceptions=False)

@av.manage_exceptions('while determining whether registry {0} contains key {1}')
def registry_member(registry, key, auth=None):
    """
    registry_member(registry, key, auth=None)
    Does registry contain an entity with this key?""" 
    (_ignore, found) = lookup_registry(
        registry, key, auth, handle_exceptions=False)
    return found
